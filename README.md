...
- **BUY/HOLD/SELL Recommendations Active** — Each 60-min cycle filters predicted top-12 to only sentiment ≥ 0.0, then issues BUY for top 2 (60/40 cash split), HOLD for owned tickers past the cap, SELL for owned tickers that dropped out or turned negative. Three exit tiers: **full stop-loss** at -10%, **partial trim** (half position) at -5% to reduce bleed from losers, and **profit-taking** (half position) at +10% to lock in gains. Score-weighted BUY sizing enforces the configured max-position cap after redistribution so one name cannot exceed risk/reward bounds. Dashboard only shows eligible tickers (no negative sentiment rows). Final recommendations issued with `EXECUTE BY HH:MM UTC` only when gate expired + market open.
